...
public class LogExpZSpline extends ExpectedModel {
    private final PolynomialSplineFunction mean, std;
    private final double nearDiagonalSignal;
    private final float maxX;

    public LogExpZSpline(MatrixZoomData zd, NormalizationType norm, Chromosome chrom, int res) {
    ...
    private PolynomialSplineFunction[] fitDataToFunction(MatrixZoomData zd, NormalizationType norm, int maxBin, int[] maxDist) {
        ...
        maxDist[0] = Math.max(maxDist[0], (int) Math.expm1(avgDistances[avgDistances.length - 1]));

        PolynomialSplineFunction p1 = new SplineInterpolator().interpolate(avgDistances, avgExpected);
        PolynomialSplineFunction p2 = new SplineInterpolator().interpolate(avgDistances, avgExpectedStds);
